...
class BizController extends Controller
{
    ...
    {   
        ...
        if(!session('account_info')){
                ...
                if($account_info['type']=='1'||$account_info['type']=='2'){
                    header("Location:".url("biz/entrance"));
                }else{
                    return view('biz/index',compact('no_include'));
                }
    ...
    public function entrance(Request $request){
        ...
        return view('biz/entrance',['n_location_name'=>$n_location_name,'n_location_id'=>$n_location_id,'no_include'=>1,'location_names'=>$location_names,'title'=>'车堂盛世-系统托管,解放老板']);
    }
    ...
    public function login(){
        if(!session('account_info')){
            ...
            if(cookie("account_name")){
                ...
                $account_pwd  = passport_decrypt(cookie("account_password"),'17cct_com_biz_login_key');
            }
            return view('biz/login',compact('no_include','title',"account_name","account_pwd"));
        }else{
    ...
    public function ajax_login(Request $request){
            ...
            if(!$account_name){
                return ['status'=>0,'info'=>'用户名不能为空!','data'=>0];
            }
            if(!$account_password){
                return ['status'=>0,'info'=>'密码不能为空!','data'=>0];
            }

            $account = DB::table('supplier_account')->where([['account_name',$account_name],['is_effect',1],['is_delete',0]])->first();
            $account = $this->objectToArray($account);
            ...
            if($account['account_password'] == md5($account_password)){
                ...
                $account_locations = DB::table('supplier_account_location_link')->where('account_id',$account['id'])->get()->map(function ($value) {
                    return (array)$value;
                })->toArray();
                ...
                DB::table('supplier_account')->where('id', $account['id'])->update($d);
                if($account['type'] == '1'){
                    $redirect_url = url('biz/entrance');
                    if(session('redirect_url')){
                        $redirect_url = session('redirect_url');
                    }
                    // $this->ajaxReturn(0,$redirect_url,1);
                    return ['data'=>0,'info'=>$redirect_url,'status'=>1];
                }
                // $this->ajaxReturn(0,U('Biz/index'),1);
                return ['data'=>0,'info'=>url('biz/index'),'status'=>1];

            }else{
                return ['data'=>0,'info'=>'用户名或密码错误!','status'=>0];
    ...
    **/
    ...
    public function authorize_add(){
        ...
        if(!$open_id){
            $this->ajaxReturn(U('Biz/authorizes'),'请重新授权',0);
        }

        $info = M('pms_weixin')->where(array('relation_id'=>$location_id,'open_id'=>$open_id,'type'=>1))->find();

        if($info){
            $this->ajaxReturn(U('Biz/authorizes'),'您已授权，无需重复授权',0);
        }
        ...
        if($result){
            $this->ajaxReturn(U('Biz/authorizes'),'授权成功',1);
        }else{
            $this->ajaxReturn(U('Biz/authorizes'),'授权失败，请重新授权',0);
        }
    }
    ...
    public function authorize_del(){
        ...
        if(!$open_id){
            $this->ajaxReturn(U('Biz/authorizes'),'信息不存在',0);
        }
        ...
        if($result){
            $this->ajaxReturn(U('Biz/authorizes'),'取消授权成功',1);
        }else{
            $this->ajaxReturn(U('Biz/authorizes'),'取消授权失败',0);
        }
    }

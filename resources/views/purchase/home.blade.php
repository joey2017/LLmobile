...
.purchasesvg{background-image: url({{asset('images/user_center.svg')}}); background-repeat: no-repeat;}
...
.searchtab span{position: absolute; width: 30px; height: 30px; display: block; top: 2px; left: 2px; background:url({{asset('images/searchico.svg')}}) no-repeat  2px 4px; background-size: 22px;}
...
/*导航*/
.purindex{background-image: url({{asset('images/purindex.svg')}});background-repeat: no-repeat;background-size: 207px;}
...
<script type="text/javascript">
	...
	function ajax_get_qualitygoods(){
	    ...
	    $("#load").show();
	    stop=false;
	    $.get("{{url('purchase/ajax_get_qualitygoods')}}",{"p":currentpage}
	  	,function(html){
	          if(html!=""){ 
	            if(currentpage==0) {

...
<div class="container-fluid" >
    ...
    @if($oi['pay_status'] == 0 && $oi['type'] == 0)
        <div class="row ddxq">
            <div class="col-xs-10" style="margin-bottom:10px;">
            	<img src="{{asset('images/weixin.png')}}" width="30" height="30" class="wxico">
...
<div class="container-fluid" >
    <div class="row">
            ...
            <div class="col-xs-12" style="margin-top:25px;"><center><button type="button" class="btn btn-danger btn-lg" @if($oi['pay_status'] == 0 && $oi['type'] == 0) id="submit" @else id="confirm" @endif style="padding-left:40px;padding-right:40px;">确认订单</button></center></div>
...
<script type="text/javascript">
    ...
    $('#submit').click(function(){
        ...
        if(pay_mode==2){
            window.location.href="{{url('pay/purchase_go_pay')}}?id={{$oi['id']}}";
        }else{
            ...
            $.ajax({
                    ...
                    success:function(data){  
                        MsgBox(data.msg)
                        if(data.status == 1){
                           window.location.href="{{url('purchase/pay_back')}}?id={{$oi['id']}}";
                        }else{
    ...
    $('#confirm').click(function(){
         $('#confirm').attr('disabled',true);
          $.ajax({
                    url:"{{url('purchase/confirm_order')}}",
                    type:"post",
                    data:{'id':{{$oi['id']}}},
                    dataType:"json",
                    success:function(data){ 
                        MsgBox(data.msg)
                        if(data.status == 1){
                           window.location.href="{{url('purchase/pay_back')}}?id={{$oi['id']}}&type=confirm";
                        }else{
